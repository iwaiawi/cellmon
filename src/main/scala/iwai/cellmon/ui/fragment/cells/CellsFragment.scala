...
class CellChangesAdapter(changeItems: Seq[CellChangeViewModel], listener: RecyclerClickListener[CellChangeViewModel])
  ...
  override def onBindViewHolder(viewHolder: CellChangesViewHolder, position: Int): Unit = {
    ...
    runUi {
      (viewHolder.changeAt <~ tvText(item.change.get.changeAt.toLocaleString)) ~
        ...
        (viewHolder.cell <~ tvCell(item.change.get.cell))
    }
  ...
  def tvCell(cell: Cell): Tweak[TextView] = cell match {
    case GsmCellLocation(lac: Int, cid: Int, psc: Int) => tvText("GSM(lac: %05d, cid:%09d, psc:%03d)".format(lac, cid, psc))
    case LteCellIdentity(mcc: Int, mnc: Int, ci: Int, pci: Int, tac: Int) => tvText("LTE(mcc:%03d, mnc:%02d, ci:%09d, pci:%03d, tac:%05d)".format(mcc, mnc, ci, pci, tac))
    case WcdmaCellIdentity(mcc: Int, mnc: Int, lac: Int, cid: Int, psc: Int) => tvText("WCDMA(mcc:%03d, mnc:%02d, lac:%05d, ci:%09d, psc:%03d)".format(mcc, mnc, lac, cid, psc))
    //		case CellIdentities(ids: Set[CellIdentity]) => ids.head match {
    //			case LteCellIdentity(mcc: Int, mnc: Int, ci: Int, pci: Int, tac: Int) => tvText("LTE(mcc:%03d, mnc:%02d, ci:%09d, pci:%03d, tac:%05d)".format(mcc, mnc, ci, pci, tac))
    //			case LteCellIdentity(mcc: Int, mnc: Int, ci: Int, pci: Int, tac: Int) => tvText("LTE(mcc:%03d, mnc:%02d, ci:%09d, pci:%03d, tac:%05d)".format(mcc, mnc, ci, pci, tac))
    //				case id => tvText(id.toString)
    //		}
